...
// --- Leaflet 은 빌드시(SSR) 절대 import 되지 않습니다 ---
// 런타임에만 import 하기 위해 null 로 초기화
let dynamicLeaflet: any = null;
...
export default function SearchPage() {
  ...
  // 클라이언트에서만 실행: dynamicLeaflet에 react-leaflet 모듈 세팅
  const [loaded, setLoaded] = useState(false);
  useEffect(() => {
    // leafet css 로드
    import('leaflet/dist/leaflet.css');
    // react-leaflet 모듈 런타임에 import
  ...
  if (!loaded) {
    return <div className="p-4">지도 로딩 중…</div>;
  }

  // dynamicLeaflet 에서 컴포넌트 추출
  const { MapContainer, TileLayer, Marker, Popup, useMap } = dynamicLeaflet;
      ...
      <nav className="flex space-x-4">
        {regions.map(r => (
          <button
            ...
            className={`px-4 py-2 rounded ${selected === r ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          >
            {r}
          </button>
        ))}
